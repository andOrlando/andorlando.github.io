...
export function about_circles() {
  
  const distribute=t=>[
      25 * Math.sin(Math.PI*2*t),
      25 * Math.cos(Math.PI*2*t)]

  const n_big_boys = Math.round(skewed_random(1/5, 1, 3))
  const n_med_circles = Math.round(skewed_random(1/3, 2, 4))
  ...
  for(let i=0; i<n_big_boys; i++) {
    ...
    y=(y*90).toFixed(2); x=(x*50).toFixed(2)
